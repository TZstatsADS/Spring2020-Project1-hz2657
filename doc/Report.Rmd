...
According to the boxplot, we can see Pop music tend to be longer than country songs and has a larger variance, while country music tends to be shorter and has a smaller variance. 
...
Next, let's analyze the high frequency words of different genres, and check if there is a difference. 

Firstly, it's an interesting phenomenon that the most frequent word in all these four genres is 'love'. 

Secondly, we can see that Pop and Country music's frequent words in lyrics are very similar, they both include Love, baby, heart, girl/boy, eyes, world. Country songs contain more words such as home, hand. 

*Note: The frequency is calculated by the word counts/number of songs in that genre*
...
HipHops and R&B's most frequent words is also 'love'. In addition, they both include the same high frequency words such as baby, time, girl .. etc.
\

There is a distinguishable differene - HipHops contains large amounts of rude words, as shown inthe graph. For the report purpose, some words are modified with '*'. HipHops also contains words such as 'ya', 'yo', 'rock'... which are considered typical Hiphops words. These words can be used to identify HipHop music. 
\

According to the high frequency words in R&B, it is similar to pop/country songs. 
...
\
Hip hop and R&B are more similar and they tend to have stronger emotions, and Hip-hop tends to have more negative words that R&B. 
\
...
We count up the number of positive and negative words in each genre, and discover a sentiment score for each word and use the added scores to compare setiment along different genres
...
**Emotion variation:**
By comparing the sentiment score for these 4 different types of songs. 
